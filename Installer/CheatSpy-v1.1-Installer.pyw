...
class InstallerApp(tk.Tk):
    def __init__(self):
        ...
        self.installdev_button = tk.Button(self, text="Install CheatSpy (DEV)", command=self.start_installdev_thread, width=20)
    ...
    def installdev(self):
        try:
            ...
            self.log_message("Downloading CheatSpy ZIP...")
            urllib.request.urlretrieve(CHEATSPY_DEV_ZIP_URL, zip_path)
            self.log_message("Download complete!")
            ...
            requirements_path = os.path.join(install_dir, "requirements.txt")
            self.log_message("Downloading requirements.txt...")
            urllib.request.urlretrieve(REQUIREMENTS_URL_DEV, requirements_path)
            self.log_message("requirements.txt downloaded!")
